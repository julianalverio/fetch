...
class Env(object):
    def __init__(self, size):
        self.size = size
        self.state = np.random.randint(2, size=size).astype(np.float32)
        self.target = np.random.randint(2, size=size).astype(np.float32)
...
class Trainer(object):
    ...
    def prepareState(self):
        ...
        return torch.tensor(np.concatenate([state, target], axis=0)).type('torch.FloatTensor').to(self.device).unsqueeze(0)

    def addExperience(self):
        if random.random() < self.epsilon_tracker.epsilon():
            action = torch.tensor([random.randrange(self.size)], device=self.device)
        else:
            input_state = self.prepareState()
            action = torch.argmax(self.policy_net(input_state), dim=1).to(self.device)
        previous_state = self.prepareState()
        reward, done = self.env.step(action.item())
        reward_t = torch.tensor(reward, device=self.device).unsqueeze(0).unsqueeze(0)
        current_state = self.prepareState()
        self.memory.push(previous_state, action.unsqueeze(0), reward_t, current_state)
        return reward, done
    ...
    def train(self):
        ...
        for episode in range(NUM_EPISODES):
            ...
            for iteration in range(max_iterations):
                ...
                if len(self.memory) == self.memory.capacity and prefetching:
