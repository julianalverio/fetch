...
class FetchEnv(robot_env.RobotEnv):
    ...
    def _sample_goal(self):
        if self.has_object:
            ...
            # goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-0.115, 0.15 + 0.115, size=3)
            goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-0.185, -0.115, size=3)
        return goal.copy()
    ...
    def getStateAndGoal(self):
        goal = self.goal.copy().astype(np.float32)
