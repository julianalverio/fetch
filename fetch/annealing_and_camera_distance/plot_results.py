...
csv_file = open('test_epsilon_reset/seed29_results.csv', 'r')
episodes, scores, mean_scores, actual_mean_scores, remaining_anneals_list, epsilons = [], [], [], [], [], []
for row in csv.reader(csv_file):
    episode, score, mean_score, actual_mean_score, remaining_anneals, epsilon = row
    episodes.append(episode)
    scores.append(score)
    mean_scores.append(mean_score)
    actual_mean_scores.append(actual_mean_score)
    remaining_anneals_list.append(remaining_anneals)
    epsilons.append(epsilon)

df = pd.DataFrame()
df['episode'] = pd.Series(episodes)
