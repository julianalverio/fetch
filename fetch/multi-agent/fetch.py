...
class Trainer(object):
    def __init__(self, seed, task_num, anneal_count=3):
        ...
        self.policy_net1 = DQN(self.observation_space, self.action_space, self.device).to(self.device)
        self.target_net1 = copy.deepcopy(self.policy_net1)
        self.policy_net2 = DQN(self.observation_space, self.action_space, self.device).to(self.device)
        self.target_net2 = copy.deepcopy(self.policy_net2)
        self.epsilon_tracker = EpsilonTracker(self.params)
        self.epsilon_tracker1 = EpsilonTracker(self.params)
        self.epsilon_tracker2 = EpsilonTracker(self.params)
        self.optimizer = optim.Adam(self.policy_net.parameters(), lr=self.params['learning_rate'])
        self.optimizer1 = optim.Adam(self.policy_net.parameters(), lr=self.params['learning_rate'])
        self.optimizer2 = optim.Adam(self.policy_net.parameters(), lr=self.params['learning_rate'])
        ...
        self.movement_count1 = 0
        self.movement_count2 = 0
    ...
    def addExperience(self):
        ...
        if self.validGrip(object_position, gripper_position):
            ...
            reward = 0.
        # if done, restart the entire simulation
        done = False or self.movement_count == 1500
    ...
    # for when stage_count == 1
    def addExperience(self):
        if random.random() < self.epsilon_tracker.epsilon():
            action = torch.tensor([random.randrange(self.action_space)], device=self.device)
        else:
            action = torch.argmax(self.policy_net1(self.state), dim=1).to(self.device)
        faction_converted = self.convertAction(action)
        self.env.step(action_converted)
        self.movement_count1 += 1
        next_state = self.preprocess(self.env.render(mode='rgb_array'))
    ...
    def addExperience(self):
        if self.stage_count == 0:
            ...
            if random.random() < self.epsilon_tracker1.epsilon():
                action = torch.tensor([random.randrange(self.action_space)], device=self.device)
            else:
                action = torch.argmax(self.policy_net1(self.state), dim=1).to(self.device)
        elif self.stage_count == 2:
            if random.random() < self.epsilon_tracker2.epsilon():
                action = torch.tensor([random.randrange(self.action_space)], device=self.device)
            else:
                action = torch.argmax(self.policy_net2(self.state), dim=1).to(self.device)
        ...
        if self.stage_count == 0:
            ...
            done = done or self.movement_count1 == 1500
        elif self.stage_count == 2:
            done = done or self.movement_count2 == 1500
        ...
        if done:
            if self.stage_count == 0:
                ...
                self.memory1.push(self.state, action, torch.tensor([reward], device=self.device), None)
            elif self.stage_count == 2:
                self.memory2.push(self.state, action, torch.tensor([reward], device=self.device), None)
            ...
            if self.stage_count == 0:
                ...
                self.memory1.push(self.state, action, torch.tensor([reward], device=self.device), next_state)
            elif self.stage_count == 2:
                self.memory2.push(self.state, action, torch.tensor([reward], device=self.device), next_state)
            self.state = next_state
        return done
    ...
    def train(self):
        ...
        while True:
            ...
            if done:
                ...
                print('Showing stats for episode:', self.episode)
